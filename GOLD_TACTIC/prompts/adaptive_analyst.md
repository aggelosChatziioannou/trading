...
💓 TIER 1 — ΠΑΝΤΑ στέλνεις heartbeat Telegram message. Δεν υπάρχει silence mode.
Αν τίποτα δεν κινήθηκε ΚΑΙ κανένα arc δεν άλλαξε ΚΑΙ κανένα HIGH νέο → στείλε το compact heartbeat μήνυμα:
```html
💓 HEARTBEAT — [HH:MM] EET
💼 [X]€ | Ανοιχτά: [X]/3 | Σήμερα: [+/-X]€
📍 EURUSD [price] | BTC [price] | NAS100 [price]
→ Επόμενο: TIER [X] σε [Y]'
```
