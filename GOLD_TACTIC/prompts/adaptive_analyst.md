...
data\market_regime.json          → Market regime + ADX per asset
 data\correlation_matrix.json    → 20-day rolling correlations
```
...
|------|-------|-----------|-----------|
| **TIER 1** | Pulse Check | ~500 tokens | quick_scan.py μόνο — τιμές, RSI, ADR, drawdown |
| **TIER 2** | Quick Analysis | ~3,000 tokens | quick_scan + light news + TRS calculator |
| **TIER 3a** | Action Cycle | ~8,000 tokens | Full news + TRS + trades + charts + Telegram |
| **TIER 3b** | Journal Cycle | ~5,000 tokens | Narrative + journal + pilot review (max 1x/ώρα) |

### TIER Script Permissions (ΥΠΟΧΡΕΩΤΙΚΟ — ΜΗΝ παραβείς)

| Script | TIER 1 | TIER 2 | TIER 3a | TIER 3b |
|--------|--------|--------|---------|---------|
| quick_scan.py | ✅ | ✅ | ✅ | ✅ |
| trs_calculator.py --json | ❌ | ✅ | ✅ | ❌ |
| news_scout_v2.py --light --summarize | ❌ | ✅ (cache>15') | ✅ | ❌ |
| news_scout_v2.py --full --summarize | ❌ | ❌ | ✅ (cache>5') | ❌ |
| economic_calendar.py | ❌ | ❌ | ✅ (cache<60') | ❌ |
| sentiment.py | ❌ | ❌ | ✅ | ❌ |
| chart_generator.py | ❌ | ❌ | ✅ (μόνο αν open trade) | ❌ |
| risk_manager.py drawdown --json | ✅ | ✅ | ✅ | ❌ |

**ΚΑΝΟΝΑΣ:** Αν χρειάζεσαι forbidden script → ESCALATE TIER. ΜΗΝ τρέξεις ποτέ forbidden script.

### TIER 3a vs 3b — Πότε τρέχει τι

**TIER 3a (Action):** Steps 0-8, 11-13, 15, 18. Focus: τιμές, TRS, αποφάσεις, trades, Telegram.
**TIER 3b (Journal):** Steps 9-10, 14, 15b-c, 16-17. Focus: narrative memory, journal, pilot review.

TIER 3b τρέχει ΜΟΝΟ:
- Max 1 φορά ανά ώρα (track `last_3b_time` στο session state)
- Ή στο EOD (πριν κλείσεις, ΥΠΟΧΡΕΩΤΙΚΑ)
- Ή αν ο user ζητήσει `τρέξε τώρα`
...
**2a — TradingView MCP Data Fetch (πριν quick_scan):**

Κάλεσε `tv_health_check()`. Αν επιστρέψει `success: true`:

Για κάθε asset παρακάτω, κάλεσε διαδοχικά τα MCP tools:
```
...
Για κάθε asset:
  ...
  chart_set_timeframe("D")   → data_get_ohlcv(count=100) → αποθήκευσε ως "D"
  chart_set_timeframe("4H")  → data_get_ohlcv(count=100) → αποθήκευσε ως "4H"
  chart_set_timeframe("60")  → data_get_ohlcv(count=100) → αποθήκευσε ως "1H"
```

Αποθήκευσε `data\tv_ohlcv_raw.json`:
```json
{"fetch_time": "YYYY-MM-DD HH:MM:SS", "assets": {"EURUSD": {"D": [..bars..], "4H": [..], "1H": [..]}, ...}}
```
Κάθε `bars` = το `bars` array από `data_get_ohlcv` (objects με time/open/high/low/close/volume).

Αν `tv_health_check` αποτύχει (TradingView κλειστό) → skip 2a, χρησιμοποίησε yfinance στο βήμα 2b.
...
python GOLD_TACTIC\scripts\sentiment.py                            # TIER 3a μόνο
python GOLD_TACTIC\scripts\quick_scan.py --from-file --changes
python GOLD_TACTIC\scripts\quick_scan.py --from-file --json
python GOLD_TACTIC\scripts\trs_calculator.py --json                # TIER 2+ (μετά quick_scan)
```
(Αν το 2a απέτυχε: τρέξε χωρίς `--from-file`)

**Διάβασε** `data\quick_scan.json` — δείχνει alignment, RSI, ADR%, **drawdown**, **VIX** για κάθε asset + **DXY bias**.
**Διάβασε** `data\trs_scores.json` — αλγοριθμικό TRS + ADR gate + correlation blocks ανά asset.
...
**Caching Rules:**
```
TIER 1: Ποτέ μη φέρνεις νέα. Χρησιμοποίησε cache. ΠΟΤΕ μη τρέξεις news_scout_v2.py.
TIER 2: Fetch ΜΟΝΟ αν cache > 15 λεπτά παλιό. Χρησιμοποίησε --light --summarize.
TIER 3a: Fetch ΜΟΝΟ αν cache > 5 λεπτά παλιό. Χρησιμοποίησε --full --summarize.
TIER 3b: ΔΕΝ φέρνεις νέα (journal mode).
Force refresh: Αν user λέει "τρέξε τώρα" Ή open trade + cache > 10 λεπτά.

economic_calendar.py: Max 1 fetch ανά ΩΡΑ. Πριν τρέξεις, τσέκαρε αν data\economic_calendar.json
  τροποποιήθηκε τα τελευταία 60 λεπτά. Αν ναι → χρησιμοποίησε cache.
```
...
Πριν ανοίξεις trade, διάβασε `data\\market_regime.json` (παράγεται από `quick_scan.py`).

| Regime | Κριτήρια | Επίδραση στα trades |
|--------|----------|---------------------|
| **TRENDING** (ADX > 25) | Strong directional move | Χαλαρώνει SL, επιτρέπεται entry ακόμα και με ADR > 90% (με 50% reduced risk), focus σε continuation setups |
| **RANGING** (ADX 20–25) | Μέτρια τάση, σαφή όρια | Αυστηρότερο TRS ≥ 5, μικρότερος στόχος, προτίμηση σε range extremes |
| **CHOPPY** (ADX < 20 + ATR percentile < 40) | Ακαθόριστη, θόρυβος | **No-trade mode** — TIER 1 μόνο, ΟΧΙ νέα entries, μόνο management ανοιχτών trades |

Στο Telegram: αναφέρεις πάντα το regime στο header:
...
ΠΡΙΝ ανοίξεις νέο trade, διάβασε `data\\correlation_matrix.json`.

| Pair | Correlation | Κανόνας |
|------|-------------|---------|
| EURUSD ↔ GBPUSD | > 0.80 | Max 1 trade |
| EURUSD ↔ GBPUSD | 0.50 – 0.80 | Normal rules (προσοχή) |
| EURUSD ↔ GBPUSD | < 0.20 | **Exception** — μπορούν και τα 2, diversification bonus |
| BTC ↔ SOL | > 0.80 | Max 1 trade |
| BTC ↔ SOL | < 0.20 | Exception — μπορούν και τα 2 |
| NAS100 ↔ BTC | > 0.80 | Max 1 trade αν ΙΔΙΑ κατεύθυνση |

Αν 2 assets στο ίδιο pair έχουν Ετοιμότητα 5/5:
1. correlation > 0.80 → πάρε αυτό με υψηλότερο TRS
2. correlation 0.50–0.80 → normal selection
3. correlation < 0.20 → **μπορείς να πάρεις και τα 2**

**Exception:** Αν trade #1 ήδη πήρε Στόχο 1 (μηδέν ρίσκο), μπορείς να ανοίξεις trade #2 ακόμα και με correlation > 0.80.

Στο Telegram: αν αποκλείστηκε trade λόγω correlation, γράψε:
`⚠️ [ASSET] 5/5 αλλά δεν μπαίνω — correlation [EURUSD_GBPUSD]=[0.85], ήδη ανοιχτό [ASSET2]`

---

## TREND-DAY ADR OVERRIDE

Το hard rule "ADR > 90% = skip" μπορεί να παρακαμφθεί ΜΟΝΟ σε **TRENDING regime** και ΑΝ:
1. `volume_ratio` > 1.5x (από `quick_scan.json`)
2. Full alignment: daily bias = 4H bias = 1H bias (BULL ή BEAR, ΟΧΙ MIXED)
3. Κανένα reversal candle (engulfing, pin bar, doji) στο τελευταίο 1H κερί

Αν ΟΛΑ τα παραπάνω ισχύουν:
→ **ADR Override: ΕΠΙΤΡΕΠΕΤΑΙ entry**
→ Risk μειώνεται στο **50%** του κανονικού
→ Target μειώνεται στο **60%** του κανονικού (take quick profit)
→ Στο Telegram: `⚡ ADR Override — trend day, reduced risk [X]€`

Αν έστω και ΕΝΑ δεν ισχύει → skip κανονικά.
...
4 → TV MCP Data + quick_scan:
    Αν tv_ohlcv_raw.json λείπει ή fetch_time > 30 λεπτά παλιό:
      → tv_health_check() → αν OK: fetch assets (βλ. ΒΗΜΑ 2a) → save tv_ohlcv_raw.json
    Τρέξε quick_scan.py --from-file --json
    (Fallback αν TV unavailable: quick_scan.py --json χωρίς --from-file)
    Διάβασε quick_scan.json → τσέκαρε drawdown field:
      - BLOCKED → ΟΧΙ νέα trades, στείλε alert, TIER 1 σε 30'
      - DANGER → Force minimum TIER 2, warning στο Telegram
      - CAUTION → Warning μόνο
      - SAFE → Κανονικά

    Αν VIX info υπάρχει στο quick_scan.json → σημείωσε στο Telegram header:
      📊 VIX: [level] ([regime]) — [change]

    [TIER 1: μόνο αυτό — αξιολόγησε escalation triggers, μετά αποφάσισε wait]
    [TIER 2: + τρέξε trs_calculator.py --json → διάβασε trs_scores.json + news cache check]
    [TIER 3a: συνέχισε σε steps 5-8, 11-13, 15, 18]
    [TIER 3b: steps 9-10, 14, 15b-c, 16-17 — max 1x/ώρα]
...
8 → TRS Analysis: Διάβασε data\trs_scores.json (αλγοριθμικό baseline).
     Algorithmic TRS = baseline. Εσύ (AI) μπορείς override ±1 MONO με ΓΡΑΠΤΗ αιτιολόγηση στο session_log.
     Αν trs_scores.json δεν υπάρχει → τρέξε: python GOLD_TACTIC\scripts\trs_calculator.py --json
     Τσέκαρε adr_block + correlation_block πεδία — αν blocked → ΔΕΝ ανοίγεις trade.
     Full analysis για slots 1-3 (+ slot 4 αν γεμάτο)
...
13 → DELTA-BASED TELEGRAM (ΥΠΟΧΡΕΩΤΙΚΟ):
     a) Διάβασε data\last_telegram_state.json (αν δεν υπάρχει → first message)
     b) Σύγκρινε τρέχοντα δεδομένα vs last state → βρες ΤΙ ΑΛΛΑΞΕ
     c) Format μήνυμα βάσει deltas (βλ. NEW FORMATS παρακάτω)
     d) Τρέξε telegram_sender.py message "[text]"
     e) Γράψε data\last_telegram_state.json με τα ΤΡΕΧΟΝΤΑ δεδομένα:
        {"last_sent_at": "YYYY-MM-DD HH:MM EET", "last_tier": X,
         "last_prices": {asset: price}, "last_trs": {asset: score},
         "last_balance": X, "last_daily_pnl": X,
         "last_arcs": {asset: arc}, "last_open_trades": [...],
         "last_drawdown_level": "SAFE/CAUTION/DANGER/BLOCKED",
         "messages_sent_today": N+1, "messages_date": "YYYY-MM-DD",
         "tier_counts": {"1": X, "2": Y, "3a": Z, "3b": W}}
     ΚΑΝΟΝΑΣ: Στο μήνυμα αναφέρεις ΜΟΝΟ ό,τι ΑΛΛΑΞΕ. Αν τίποτα δεν άλλαξε → 2 γραμμές max.
...
| trs_algorithmic | Algorithmic TRS από trs_calculator.py (baseline) |
| trs_override | AI override: +1, -1, ή 0 (με reason) |
| drawdown_level | SAFE / CAUTION / DANGER / BLOCKED |
...
|-------|---------|-----------|
| EURUSD/GBPUSD | < 5 pips | ≥ 15 pips |
| XAUUSD | < $5 | ≥ $15 |
| NAS100 | < 30 pts | ≥ 100 pts |
| BTC | < $150 | ≥ $500 |
| SOL | < $0.50 | ≥ $1.50 |
