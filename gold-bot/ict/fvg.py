"""Fair Value Gap (FVG) detection, fill tracking, and Inversion FVG (IFVG)."""
...
class FVG:
    ...
    inverted: bool = False  # IFVG: filled FVG now acting as opposite S/R
    ...
    for i in range(2, len(df)):
        ...
        if lows[i] > highs[i - 2]:
            fvgs.append(FVG(
                ...
                top=float(lows[i]), bottom=float(highs[i - 2]), index=i - 1,
            ))
        # Bearish FVG
        elif highs[i] < lows[i - 2]:
            fvgs.append(FVG(
                timestamp=df.index[i - 1], direction="bearish",
                top=float(lows[i - 2]), bottom=float(highs[i]), index=i - 1,
            ))
    return fvgs
...
def update_fvg_status(
    ...
    """Update FVG fill/inversion status. Returns active FVGs."""
    active = []
    for fvg in fvgs:
        ...
        if not fvg.filled:
            # Fill = price enters the FVG zone
            if fvg.direction == "bullish" and low <= fvg.top:
                fvg.filled = True
            elif fvg.direction == "bearish" and high >= fvg.bottom:
                fvg.filled = True

        # IFVG: filled FVG where price pushes through the other side
